...
def load_cupl_descriptions(hparams, classes_to_load=None):
    dataset_name = hparams['dataset']
    textnames=load_classes(hparams['dataset'])
    with open(f"prompts/{dataset_name}/cupl.json") as f:
    ...
    with torch.no_grad():
        ...
        for i in range(len(textnames)):
            ...
            if i == 0:
                print(texts)
            cupl_descriptions[textnames[i]]=texts


    return cupl_descriptions
